...
test('Latest and down effectively runs all but one migrations', async t => {
  await runMigrations(options)
  const pgClient = await createPgClient()
  await pgClient.query(`INSERT INTO users (name, display_name) VALUES ('andre', 'André Gaul');`)
  const result = await runMigrations({ ...options, mode: 'down' })
...
test('Latest, down and up effectively runs all migrations', async t => {
  await runMigrations(options)
  const pgClient = await createPgClient()
  await pgClient.query(`INSERT INTO users (name, display_name) VALUES ('andre', 'André Gaul');`)
  await runMigrations({ ...options,	mode: 'down' })
  const result = await runMigrations({ ...options, mode: 'up' })
  t.deepEqual(result, [migrations[1]])
  const rawMigrations = await pgClient.query(`SELECT * from ${TABLE} ORDER BY sort`)
  t.deepEqual(rawMigrations.rows.map(v => v.id), migrations.map(v => v.id))
  const rawUsers = await pgClient.query(`SELECT * from users`)
  t.deepEqual(rawUsers.rows, [{ id: 1, name: 'andre', display_name: null }])
  await pgClient.end()
})
